...
| §E | **SHARED MODULES** — seventeen: `AmrTick`, `AmrCache`, `AmrKpi`, `AmrCube`, `AmrDeckSource`, `AmrPvSlide`, `AmrProgress`, `AmrBoot`, `AmrFresh`, `AmrStamp`, `AmrSlide`, `AmrFuelExec`, `AmrSegSlide`, `AmrRmxSlide`, `AmrHover`, `AmrSugChip`, `AmrLookupEdit` |
...
- **`AmrLookupEdit`** — the add-rows form for `REGION LOOKUP`, `PRODUCT MASTER` and
  `EXTRAS LOOKUP`. Was the Overview's; shared the moment the Deck Builder's pre-flight
  needed the same form. A page mounts a namespace, says where a section's form goes and
  what a write invalidates — see §8.
...
- **Product Segment workbook** — `Slide Segment MTD` / `YTD` and `Slide Product <Market> MTD` /
  `YTD`, all pre-aggregated by QlikView. **Only the `Slide Product` half is read now**, by the
  Product Segment page's *Product category* card (`SB_getProductTable`) and by the Overview's
  Product Category panel. Nothing reads `Slide Segment` any more — that table is computed from
  the Ready-Mix raw tabs — but the sync still writes both, because a tab nobody reads costs a
  column write and a tab that stops being written cannot be turned back on without a re-export.
...
| `stale`, not `built` | a file exists but the shape version has moved, so it reads as absent | the browser reads it — those months are not in the cube at all |
| `stale` **and** `built` | readable, and the workbook behind it has changed | the hourly trigger, or the pill's **Reload history** |
...
**The line the browser acts on is `built`, and for a while it was `first`, which is how the
slider lost two calendar years.** `built:false` is `ovcHistFile_` returning null: those months
are not in the cube, so nothing on the page can reach them. Bumping `OVCUBE_SHAPE_VER_` v5 → v6
(2026-08-27) therefore took 2023 and 2024 off the Overview's Months slider for everybody at
once — silently, with the pill still reading *"History loaded"*, because `histStale()` only
counts the readable kind. `histPending()` now takes any linked book that is not `built`; a book
that IS built and merely out of date stays the trigger's and Reload's, because its months are on
the page and re-reading throws away the cube to fix something nobody can see. The foreground
cost the `first`-only rule was written against is gone for its own reason: `histBootReady()`
releases the boot step as soon as the cube can answer the opening window, and
`maybeBuildHistory()` does not start until every block already built has landed — so an
unreadable book is re-read one era per run, behind a pill nothing waits on.
...
**Which is why the Product Segment page's *Product category* card is offered for one report
month and no other.** Every other table on that page is computed from the raw tabs and follows
the month picker; that one is the Slide Product tabs, so it can only be last closed month's.
`SB_getProductTable` returns `monthIdx` (`ovSegMonth_`, the calendar's last closed month) and
the page draws the card only when the picker is on that month — the same rule, and for the same
reason, as the Overview's `pcatFits()`. In August it is July's card or no card.
...
- **The split happens once, at the bundle, and every caller inherits it.** `splitNonConc_`
  ...
  to one question) — and it reaches the rule as **`RMX_NS.isNonConc`**, because `rmxNonConc_`
  is written flush-left but lives INSIDE the `RMX` closure and a top-level function cannot say
  that name (fixed 2026-08-27; it threw `rmxNonConc_ is not defined` on the first extras row
  of every closed book). Eleven places sum those two streams — a per-caller filter is a per-caller
...
- PPI accuracy: RMX rows expose `rfiBase`/`facBase` for exact subset PPI. AGG all-markets
  returns the exact `aggAll.ppi`; a single market is exact; **a 2+ market subset off the plain
  report** would be a CY-revenue-weighted blend of the markets' own indices, which is why the
  page does not ask the plain report for one — see the multi-select bullet below.
- **The market chips are MULTI-SELECT, and what makes that safe is which path a subset takes.**
  A chip toggles its own market, *All markets* clears the set, and un-picking the last chip is
  the same as *All* (an empty selection is not a page). They were single-select on the grounds
  that anything else would be blended or estimated, and that was true of exactly one path: the
  plain `getOverview` / `getReport` path, whose market filter is a **single string** server-side
  (`filterValue`) and whose subset PPI is the blend above. Every other path already took a list —
  `getCrossReport` and the browser's own cross-filter engine filter `MARKET` as an array,
  `trendFilters()` hands the cube every selected market, `loadCustomers()` has fanned out per
  market and merged since it was written, `rmxTotals()` and `renderProdCat()` sum per market
  exactly. So a subset is **routed off the path that would have to approximate**: `multiMarket()`
  makes `xfActive()` true, the cross-report owns every Aggregates panel and computes every figure
  (PPI included) on the selection itself, and `loadDims` / `loadPM` return early rather than
  asking a one-market report a many-market question and caching the answer under the set's key.
  Ready-Mix needs no such routing — its tab is cross-report-fed already — so `mktOk_` takes a
  **list** as well as a name, and `mktTag_` sorts it into the cache key. One market, or all of
  them, is unchanged and still comes off the plain report. Two things stay single-market by
  nature and say so in code: the Land / Docks refine (Southwest only) and the Ready-Mix
  submarket first paint, which `getOverview` carries per market. **The filter drop-downs get a
  cube fallback out of it**: in window mode neither the cross-report (a *period* report) nor the
  plain report (not fetched for a subset) can fill them, so `xfOptionsFor()` falls back to
  `cubeGroups()` — the same fields off the fact table the window is already drawn from, and
  scoped to the SELECTION rather than to the dictionary, because offering a value with no rows
  behind it is a filter that empties the page for no visible reason.
...
- **And therefore the SAP / USGAAP cards are the PREV-MONTH picks' and nobody else's.**
  ...
  instead — the whole strip sat inside a `!winMode()` guard, and both Prev-month picks are
  windows — so the one statement the workbook holds was printed under a heading for the month
  *after* it, and it vanished the moment anybody picked the month it was actually about. The
  card's own label said so out loud (*"year to date · the closed month, expected Jul 2026"*) and
  nothing else did. `syncWinPanels()` is the outer gate and `renderAggKpis` / `renderRmxKpis`
  bail on the same `isPrevPick()` test, so the two cannot disagree; a **dragged** window still
  hides them for the older reason, which is that a statement exists per month or per year and a
  five-month span is neither. The two Prev-month buttons carry a `title` saying what they
  unlock, because a whole panel that appears and disappears with no word anywhere is what made
  the old arrangement so hard to see. Ready-Mix had it wrong in the same direction and is fixed
  the same way.
- **The QlikView half of that strip reads the CUBE, not `getOverview`.** The strip is a
  Prev-month pick's, a Prev-month pick is a window, and `getOverview` answers for the anchor —
  one month later. So the companion cards (`kpiQlik()`) come off `cubeWinTotals()` for the span
  actually on screen. Two ASPs side by side on one card, both correct, describing different
  months, is not a comparison.
- **The workbook counts in thousands and the cube counts in ones.** `Total Product Revenue` and
  `Total Sales Volume` are **k$ and kt** on a plant statement, and the card proves it rather
  than assuming it: the AGG CENTRAL year-to-date block reads 149,830 and 9,070, and
  149,830 ÷ 9,070 = **16.52**, which is the `$/t` the same block carries and the number the ASP
  card prints. `FMT.thou` then divides by a further 1,000 for the *(K)* label. Ready-Mix is the
  same shape — `Units Sold` is Km³, and `RC/m³` is kRC ÷ Km³. So the QlikView volume cards
  divide the cube's plain tonnes and cubic metres by `KPI_UNIT_SCALE` first, or the two volume
  figures would sit side by side three orders of magnitude apart.
- **Manitoba and Saskatchewan are a second workbook, and "Central Canada" does not reach
  them.** `KPI_BOOK_OF` is the whole of it: those two markets are statements in the MB/SK
  EBITDA book and every other market is in the main one — so **no region tab in the main book
  carries them, `AGG CENTRAL` included**, however much the name suggests otherwise. The strip
  named the tab and the page named the region and neither said the two provinces were outside
  both; `aggMbskNote()` / `rmxMbskNote()` now say it under the cards, on both tabs.
...
- **The row export ships the index's INGREDIENTS, never the index.** `⭳ Export rows` writes
  the fact table the page is drawn from as a CSV — one line per (month × dimension tuple),
  the raw columns, the looked-up ones, and both years side by side. Three rules make it
  reconcilable rather than merely similar:

  **The grain is the pair, not the source row.** A source row carries ONE year (§7), so
  "Vol % per row" has no answer at that grain — every current-year line would divide by a
  zero prior year. `AmrCube.exportRows` joins the two years on (tuple × month). It costs
  nothing: the source rows are already unique on (dims + year + month), so the join pairs
  one row with one row and aggregates none of them away.

  **PPI and CPI are pooled, so the file carries `weight` and `factor` and not the index.**
  There are a few hundred plant × material pairs behind tens of thousands of lines; a "PPI"
  column could only be its pair's index repeated, and the first person to average it has a
  number this page will not agree with — which is the same "any new per-row ratio is a bug"
  rule §7 states. Each line's weight is its **own CY revenue** on a covered pair, and its
  factor is that weight × the *pair's* ASP move, so summing a pair's lines gives the pair
  ...
  **Twelve months, and the cap is the point rather than a limit of the code.** Every line
  ...
  year leaves those cells **blank rather than zero**, because a zero prior year turns every
  ...
  **`.ghost` is the navy-header button and renders white-on-white on a light card.** The
  ...
  prior-year only, which is why a per-row YoY was never available. `Rev CY` sums to
  $194,456,116.98, the QlikView file's own totals row to the cent. Wholly ungated, CPI reads
  **141.719%** — §7's own recorded figure for that window, reproduced by the export path.
...
**The missing rows can be added from the dialog itself.** Each lookup finding carries an
**+ Add these N rows** button, and the form that opens under it is the *same one* the
Overview's data-quality sheet has always used — extracted to `AmrLookupEdit` (§4) the moment a
second page wanted it, rather than copied. It is the right place for it: the pre-flight finds an
unmapped plant at the one moment somebody is about to publish 43 slides built on it, and sending
them to another page to fix it costs the deck they are standing in, because switching pages
unmounts the Deck Builder and every picture with it. The **Open the page** buttons stay as the
fallback — for the per-value revenue, the downloads, and the two lookups with no append API.

**A save ends that render rather than continuing it**, and that is the honest answer rather than
a cautious one: the write bumps the page generation, so every server cache key is unreachable and
the cube is rebuilding — but the nearer reason is that *the market a plant lands in has just
changed*. Every table the deck would photograph is about to be re-read into different buckets and
every picture already taken used the old ones, so carrying on would photograph last minute's
numbers under this minute's lookup, which is the exact failure the dialog exists to prevent. So
the adapters are dropped, every picture is dropped, the mapping counts are forgotten, and the
dialog's primary button stops offering a render and says *Done — press Render again*. One more
press re-runs the source check, which is what picks the new generation up.
...
| 2026-08-27 | **The SAP / USGAAP strip was on the two picks it cannot be for, the closed-year books had gone quiet, and the market chips were single-select for a reason that applied to one code path.** Four things, reported together from the Overview. **(1) The strip belongs to the Prev-month picks.** `kpiMonth()` is `anchorMonth() - 1`, which is precisely the month *Prev month (MTD)* spans and the month *Prev month (YTD)* ends on — and the whole strip sat inside a `!winMode()` guard, while `STATE.win` is true for both Prev-month picks. So the one statement the EBITDA workbook holds was drawn under *This month* and *Year to date*, a heading for the month AFTER it, and disappeared the moment anybody picked the month it was actually about. The card's own label had been saying so out loud since the month name was added — *“year to date · the closed month, expected Jul 2026”* — and nothing else did. `syncWinPanels()` is now the outer gate (`isPrevPick()`) and both renderers bail on the same test, so they cannot disagree; a **dragged** window is still hidden for the older reason, which is that a statement exists per month or per year and a five-month span is neither. Ready-Mix was wrong in the same direction and is fixed the same way. The two Prev-month buttons carry a `title` saying what they unlock, because a panel that appears and vanishes with no word anywhere is what made this so hard to see. `kpiPeriodLabel()` names the pick (*previous month*) instead of the server period behind it. **(2) The QlikView companions read the CUBE now, and a new volume card sits beside the SAP one.** The strip is a window's, and `getOverview` answers for the anchor — one month later — so `kpiQlik()` takes `cubeWinTotals()` for the span on screen: two ASPs side by side, both correct, describing different months, is not a comparison. **The units were reconciled, not assumed**: a plant statement counts **k$ and kt**, which the card proves on itself — AGG CENTRAL YTD reads 149,830 and 9,070, and 149,830 ÷ 9,070 = **16.52**, the `$/t` the same block carries and the figure the ASP card prints — and `FMT.thou` divides by a further 1,000 for the *(K)* label, so the cube's plain tonnes go through `KPI_UNIT_SCALE` first or the two volume cards would sit three orders of magnitude apart. Ready-Mix is the same shape (`Units Sold` is Km³, `RC/m³` is kRC ÷ Km³) and gets the same card. **And both strips now say what “Central Canada” does not reach**: `KPI_BOOK_OF` is the whole of it — Manitoba and Saskatchewan are statements in the MB/SK book, so **no region tab in the main book carries them, `AGG CENTRAL` included**. The old *“not affected by the cross-filters”* note is replaced by that one; its `.xf-note` CSS rule is left in place, unused. **(3) The slider had stopped offering 2023 and 2024, and the shape bump the day before is why.** `histPending()` asked for `first` — a book NOBODY has ever read — and a book whose parked file was written against an older `OVCUBE_SHAPE_VER_` is not that: it is `stale` with **`built:false`**, which is `ovcHistFile_` returning null, which means those months are not in the cube at all. `v5 → v6` (row above) therefore took two calendar years off the Overview for everybody at once, silently, with the pill still reading *“History loaded”* because `histStale()` only counts the readable kind. It now reads any linked book that is not `built`; a book that IS built and merely out of date stays the hourly trigger's and Reload's, because its months are on the page and re-reading throws away the cube to fix something nobody can see. The foreground cost the `first`-only rule was written against is gone for its own reason: `histBootReady()` releases the boot step as soon as the cube can answer the opening window, and `maybeBuildHistory()` waits for every block already built to land. **(4) The market chips are multi-select.** They were single-select on the grounds that anything else would be blended or estimated, and that was true of exactly one path — the plain `getOverview`/`getReport` one, whose market filter is a **single string** server-side and whose subset PPI is a CY-revenue-weighted blend. Every other path already took a list: `getCrossReport` and the browser's own cross-filter engine filter `MARKET` as an array, `trendFilters()` hands the cube every selected market, `loadCustomers()` has fanned out per market and merged since it was written, `rmxTotals()` and `renderProdCat()` sum per market exactly. So the subset is **routed off the path that would have to approximate**: `multiMarket()` makes `xfActive()` true and the cross-report owns every Aggregates panel, computing PPI on the selection itself, and `loadDims`/`loadPM` return early rather than asking a one-market report a many-market question and caching the answer under the set's key. Ready-Mix needed the server: `mktOk_` takes a **list** as well as a name and `mktTag_` sorts it into the cache key, so `getRmxCrossReport` is one report over exactly the markets picked. `xfHasFilters()` was split out of `xfActive()` so a market-only pick does not draw an empty *“Filtered by … Clear filters”* bar, `pickMarket()` is the one place a market click is decided (chips, both breakdowns, a `MARKET` cross-filter value), and the two things that are single-market by nature say so in code — the Land / Docks refine and the Ready-Mix submarket first paint, which `getOverview` carries per market. The Markets caption *“· pick one market, or all”* is gone with the rule it described. **Not testable off-platform, as ever.** Gate run: `node --check` clean on `script.gs` and on all 30 of `app.html`'s inline script blocks, extracted to `.js` paths. **Before trusting it, in the editor and the live deployment: step all four Period buttons on both tabs and confirm the SAP strip appears on the Prev-month pair only and that its QlikView volume is the same order as the SAP one; pick two and three markets and check the Aggregates panels come off the cross-report and the Ready-Mix ones cover both markets; and watch the pill re-read the two closed-year books once and the slider grow back to Jan 23.** |
| 2026-08-27 | **The month cube's history reader died on `rmxNonConc_ is not defined`, because the rule it calls is written flush-left inside a closure.** Reported as the bare `ReferenceError`. `rmxNonConc_` (added the same day with the all-in-ASP fix two rows above) is declared at column 0 — which is what makes it read as a global — but it sits at line 9954, inside `var RMX = (function(){` … `})()`, so it is private to that module and only the other code in there can see the name. `ovcRmxExtraTab_` is a top-level function: `if (rmxNonConc_(h3)) continue;` threw on the **first extras row of every closed book**, which is `ovcHistRmx_`, which is the parked history for 2023–2025. The live half never hit it — it reads `b.extras` / `b.assoc` off the bundle, already split by `splitNonConc_` inside the closure — so the failure was confined to the one path that had just been made to rebuild (`OVCUBE_SHAPE_VER_` v5 → v6 the same day) and nothing on screen named the closure. **Fixed the way this file already hands out the rest of that rule**: `isNonConc: rmxNonConc_` on the `RMX` return object, called as `RMX_NS.isNonConc(h3)` on the line below the `RMX_NS.extraHier3()` it already went through. Copying the predicate out next to the caller was the alternative and is the thing §7 forbids — one list (`RMX_NONCONC_H3_`) and one prefix rule (`rmxH3Key_`) are what stop the cube and the bundle splitting differently. **The whole file was swept for the same shape**: 228 names are declared flush-left inside a closure, and this was the only one called from outside — the other 227 are reached through their namespace or not at all. **No cache or shape bump**: a build that threw parked nothing, and a book with no extras tabs returns before the loop, so no parked v6 file can carry the wrong split. Gate: `node --check` clean on `script.gs` at a `.js` path, plus the rule loaded out of the edited file under Node and run over the README's own trap cases — both spellings of Yard-stone-sand, the two colon spacings, the 8-character floor, and `4 : Steel Fibers` / `9 : Fuel Surcharge` staying concrete | ✅ |
| 2026-08-27 | **The Overview's Top-10 customers panel breaks down by SOLD TO as well as by customer segment, and `STATE.split` stopped being a boolean to do it.** Asked for directly: the panel had *By customer* and *Split by segment*, and the AGG Price & Volume drill-down beside it has had *Customer parent / Customer segment / Application / Sold to* all along — the sub-breakdown the Overview offered was the narrower one. Three pills now: **By customer**, **Customer segment**, **Sold to**. `STATE.split` is `''` or a KEY OF `CUST_SPLIT`, and those keys are the server's own `CUST_SECONDARY` names, so `loadCustomers` hands the pill straight to `getCustomerReport` with nothing to translate and no second list to keep in step; it stayed truthy-or-empty on purpose, so every `STATE.split ? … : …` still reads as *is the table expanded* and only the places that need the DIMENSION go through `splitDef()`. **The panel has three owners and all three had to answer the same pill** (`repaintPanel`): the server (`secondary`, already accepted `SOLD_TO` — the report needed no change), the month cube (`soldTo` is in `OVCUBE_SHAPE_.agg.dims`, so `cubeCustomers` groups on `sp.dim`), and the cross-filter report, which is the one that needed work. **The pill is not part of `xfSig()`** — it changes what one panel shows of a selection, not the selection — so a cross payload carrying only the pressed split would have cost a recompute, or a server round trip on the fallback path, to answer the other. `getCrossReport` and `xfComputeLocal` therefore build **both** child sets in the one pass (`children` stays the segment set the table reads; `childrenSoldTo` rides beside it) for the same top `XF_CHILD_CAP` rows, and the growth is small because Sold To is nearly 1:1 with Customer Parent. **Two silent-empty holes closed on the way**: a manifest built before `soldTo` was carried answers a groupBy on it with the `\u0001all` sentinel — a grand total wearing a row's clothes — so `cubeHasDim` is checked first and the sentinel dropped if it arrives anyway, and the window's subtitle says the split is not in the loaded months rather than drawing a customer with no children; and the device copy of a cross payload keys on the DATA version, which a code change does not move, so the key namespace is `xf2:` and a copy saved before this change cannot be served as one that can answer the sold-to pill. `node --check` clean on `script.gs` and on all of `app.html`'s script blocks | ✅ |
...
| 2026-08-27 | **The missing lookup rows can be added from the Deck Builder, and the form is the Overview's rather than a third copy of it.** Asked for directly. The pre-flight finds an unmapped plant at the one moment somebody is about to publish 43 slides built on it, and the answer it had was *go to another page* — which unmounts the Deck Builder and every picture with it. So each lookup finding now carries **+ Add these N rows**, and the form that opens under it is the one the Overview's data-quality sheet has always used. **It was extracted, not copied**: §E `AmrLookupEdit` is the Overview's editor moved whole — the form, its state, both write paths, the option-list trick that made the 200-row cap unnecessary (a vocabulary rendered into every row was 91,200 nodes and 5.8s; one option per select plus a fill-on-touch is 25,200 and 446ms), the harvest-before-re-render rule and the confidence chip. A page mounts a namespace and says two things only it can answer: where a section's form goes, and what a write invalidates — a cube reload on the Overview, dropped pictures on the Deck Builder. Its Cancel / Save / × are one delegated handler reading the namespace and key off `.lk-wrap`, so neither page names those controls any more; **+ Add these rows** stays the page's, because only the page knows which values a section is offering (the cube's list is not the live report's). The `.lk-*` rules moved from §A4's overview scope to §A3 on that layer's own rule, and the busy line stopped being `.ov-mini` — an §A4 class, so on any second page it was unstyled text. **A save ends that render** rather than continuing it: the write bumps the generation and the market a plant lands in has just changed, so every picture was taken with the old buckets — adapters dropped, pictures dropped, counts forgotten, and the primary button stops offering a render and says *Done — press Render again*. **Gate:** `node --check` clean on `script.gs` at a `.js` path and on all 31 of `app.html`'s inline script blocks, plus a headless Chromium pass over **both** callers against stubbed `google.script.run` — on the Overview: four sections, the form opening on the cube's values, a select filling on focus, a typed value and a picked option surviving a full `renderDq()` rebuild, a crossed-out row excluded from the write, two sections' forms open at once and Cancel closing only one, the chip resolving through the `ovlk` hover scope; on the Deck Builder: the button on each finding, the form, the save, the relabelled footer, the dropped pictures and the re-asked question on the next Render. Client-side only — no cache, shape or manifest bump, and no server figure moves | ✅ |
| 2026-08-28 | **The Product category table is back on Product Segment, for last closed month only; the Price & Volume KPI strip gained the QlikView volume card and stopped needing the Region dropdown to finish loading; the Overview's SAP strip moved up under Market summary.** Four asks, one session. **(1) The Product table returns and it is still the old source** — it has to be: `Main Raw Data` carries no product-category column, so there is nothing to recompute it from, and the `Slide Product <Market> MTD / YTD` tabs the sync still writes are the only place those rows exist. `SB.getProductTable` reads one tab per market through **`ovProdRows_`, the §8 port of this page's own column detection** rather than a second copy of it, and rolls the five markets up for Central Canada on the volume-weighted revenue and CM2 sums that port already carries — so a merged ASP is the weighted one. It is a **second server call**, not another field on `RMX_getSlideTables`, because it lives on the **segment** generation while every other table on that page lives on the Ready-Mix one, and one cache token cannot honestly track two workbooks; for the same reason its rows are held in memory for the session and never filed in the page's device cache, which is stamped with the Ready-Mix pair. The Segment table is untouched and still computed from the raw tabs. **(2) The card exists for one month and no other.** Those tabs have no month column, so they can only be last closed month's; `productShown()` compares the picker against the payload's `monthIdx` — the SERVER's last closed month — and the browser's own clock is used only to avoid a round trip that cannot match. Not "no data" but no card, no tick box and nothing on the slide, because July's figures under an August heading is the specific-looking kind of wrong. `AmrSegSlide` gets the same gate through a `product` ctx field being null, which is also why the deck, which never loads those tabs, cannot offer the card. **(3) The QlikView volume card, on Price & Volume and everything exported from it.** One line in `AmrPvSlide.kpiCardsHtml`, which is the single builder behind the on-page strip, Download KPI PNG, the slide preview, the slide PNG and the deck's `pv` rows — so all five gained it at once. The plant statement counts kt and `getReport` counts tonnes, so the total is put on the workbook's scale first (`KPI_UNIT_SCALE`), exactly as the Overview's strip already does. **(4) `if(window.renderKpis)` was the reported bug.** Reported as *sometimes the Price & Volume page doesn't load one of the numbers until I reselect the region*. On `Page_PriceVolume.html` every function was a file-scope global and that guard was always true; inside the page module `window.renderKpis` is `undefined`, so **the KPI strip was never redrawn when market data landed**. The shared EBITDA figures usually arrive first — they are read off the device while `getReport` is still out — so the strip was built with no report behind it and both QlikView cards printed "Load market data to fill this card" until the Region dropdown, the only other caller, was touched. Same bug class as the `paintImpact()` one on Ready-Mix, and the comment there now has a twin. **Plus** the Overview's `KPIs — SAP / USGAAP` panel moved above `Month by month` on both tabs — the summary, then the closed month's statement, then the breakdown of it — and the two tabs stay twins. | ✅ |
| 2026-08-28 | **The Overview exports its rows, and the two-calendar-year limit turned out to bind on only one of the two sources.** The raw workbooks do carry it — one book is one CY/PY pair, so Dec 24 → Dec 25 sits inside `histagg` and Jan 23 → Jan 25 sits in no single book — but the month cube does not: its ERAS merge already stitches Jan 2023 to the newest month with no month duplicated, and its nine dimensions ARE the raw tab's nine. So the export is built in the browser off the cube the page already holds: no server call, no payload ceiling, no new scope. **The two things that had to change shape are the two the request assumed were columns.** A source row carries one year, so per-row Vol % / ASP % do not exist and the grain moves to the pair (measured: only 7,908 of 17,839 current-year rows have a prior-year twin at full grain — a per-row YoY would have been blank on 56% of the file and −100% on it if zeros had been written). And PPI / CPI are pooled over a few hundred pairs, so the file ships `weight` and `factor` per line and `Σf ÷ Σw` is the index — exact for any slice that cannot split a pair, which is every plant- or material-derived dimension. **The span is capped at twelve months** rather than at two calendar years: per-line measures survive any span but the indices pool, and that is the rule `pyStale()` already applies to the page. Verified by driving the shipped `AmrCube.query` and `exportRows` against a cube built from the export workbook in the repo — 52 checks, including every market subset re-pooling to its own PPI and CPI exactly, `Rev CY` summing to the QlikView totals row to the cent, and the ungated CPI reproducing §7's recorded 141.719%. | ✅ |
| 2026-08-28 | **The SAP / USGAAP KPI strips were drawn under any Report month on the two pages that upload the workbook.** The EBITDA workbook is a *closed-month* statement — this calendar month minus one — and nothing inside it says so, so a strip beside any other Report month is one month's figures under another month's heading. The Overview has always answered this by hiding its SAP / USGAAP panels outright on every pick but its Prev-month pair; **Price & Volume** and **RMX · Product Segment** drew theirs on every month, and Price & Volume additionally ran the workbook's file-name check against **the picker** (`STATE.month || STATE.latestMonth`), so a correct July file was reported as the wrong file whenever the page sat on the running month — while Product Segment made **no check at all**. `AmrKpi.closedMonth()` / `closedLabel()` are now the one place that calendar rule lives. Both pages hide the strip unless their Report month IS the closed month, and they do it through `CTX.kpi.off` — **the switch the two slide builders already had** — so the on-page strip and the slide's own strip go together and neither can be exported alone; P&V's Region picker goes with it. A month not resolved yet (0, before the first payload lands) is not treated as a disagreement: the backends' own default IS the closed month. The file-name caution now runs against the closed month on both pages. **No KPI value moved** — the cards were always the closed month's; what changed is where they are allowed to appear. | ✅ |
