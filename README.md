...
| | **The rest of Ready-Mix's bare-element §A4 rules are unaudited for the same specificity trap** — `table`, `th`, `td`, `thead th`, `tfoot td`, `tr.subtotal td`. `css-reach` proves none of them escapes `#appRoot`, which is the half that bites hardest; what it cannot see is a shared `.class` rule INSIDE the page that the raised specificity now overrides. Needs a booted page and a computed-style diff against the legacy one, so it wants doing before chunk 13 deletes `Page_Rmx.html` | ☐ |
