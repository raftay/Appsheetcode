...
| | **`AmrKpiStore` in `Shell.html` has zero callers** — only its own definition and two comments, across every `.html` and `.gs`. Deliberately not ported in chunk 5. Decide by chunk 13. `AmrTick` is in the same position; check the Overview (chunks 8–9) first | ☐ |
...
| | **`.ghost` renders white-on-white in the QlikView guide** — found on all four ported pages so far and fixed in each. `Page_Segment`, `Page_TP01` and `Landing` still need checking as their chunks land | ☐ |
