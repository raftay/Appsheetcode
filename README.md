...
| Section | Was | Role |
|---|---|---|
| §1 CONFIG | `Config.gs` | `APP_CONFIG` — every sheet ID, tab name, market list, cube constants — plus `OVERVIEW`, the Settings API and `LOG_LEVEL`. **Since chunk 22 it also holds `DECK_CONFIG` and `DECK_RECIPE`**, moved up from §9: the deck's template, folder, capture resolution and slide list are configuration, and they were the deepest-buried thing in the file. **First on purpose:** IIFEs run at evaluation time, so nothing that reads config while constructing itself can precede it. Its banner is the map of every constant in `script.gs` worth changing, including the ones that stay beside the code that reads them |
...
| §9 DECK | `Deck_Backend.gs` | Deck Builder server plumbing: template geometry, create/addSlide/finish/status, validator. See §8. **`DECK_CONFIG` is in §1** since chunk 22 — what is here is the engine |
| | `Deck_Recipe.gs` | `DECK_getRecipe()`, which checks the recipe. **The recipe itself — which 43 slides the deck contains, in order — is config, and is in §1** |
...
| §C TUNABLES | *new* | `AMR_TUNABLES` — the 16:9 slide frame (size, padding, default bands, slider ceilings) and **where each of the five pages' whitespace sliders start**. Everything in it was already in the file, in the module or the page that read it; nothing new. Runs above §D and §E because their IIFEs read it while they construct, the same rule `script.gs` §1 is written to. `tests/tunables.js` is the gate |
...
| `AmrSlide` | `SlideExport.html` | The 1600×900 slide frame, whitespace sliders, full-window viewer, html2canvas PNG export, and `captureBare` for the deck. **Its four frame numbers are in §C** since chunk 22 |
...
| 2026-08-19 | **Chunk 22 — the settings a user is expected to change are at the top of both files now.** `DECK_CONFIG` and `DECK_RECIPE` moved from `script.gs` §9 to §1, unchanged: the deck template id, its destination folder, the capture resolution and **which 43 slides the monthly pack contains** sat at lines 10,281 and 10,968 of an 11,700-line file, behind the engine that reads them. On the client, `app.html` gains **§C TUNABLES** — the 16:9 slide frame (`W H PAD BARE_PAD`, the default bands and the slider ceilings, lifted out of `AmrSlide`) and **where each of the five pages' whitespace sliders start**, which were five literals scattered from line 8,931 to line 14,294. §1's banner and §C's are now the map of every constant in each file worth changing, including the ones that deliberately stayed put and why | ✅ done |
| 2026-08-19 | **`OVERVIEW` was labelled `NOT USED` and is read on every Overview load.** The banner came across from `Config.gs` verbatim in chunk 12 — which is exactly how a wrong comment survives a merge — while `getOverview` takes its whole market list from `OVERVIEW.MARKETS`, the page footer reports any PV market missing from it as "unmapped", and `app.html` names the object by name in that hint. Deleting it on the strength of its own comment would have emptied the Executive Overview. The comment is corrected and declared to `gsparity.js`; nothing else about the object changed | ✅ done |
| 2026-08-19 | **`tests/tunables.js`** — §C is what the pages said, proved against `app.html` at `c24ccd6`: every literal it was lifted from still exists at that commit, every number in §C equals it, each page asks for **its own** row (a copy-pasted page id would silently give one page another page's slide), no page mounts sliders from a literal any more, §C evaluates with nothing but `window` — it runs above §D and §E because their IIFEs read it while they construct — and `ws()` hands out a copy rather than the table, since `mountControls` keeps the object it is given and the sliders write into it. Mutation-tested two ways. Also verified in real Chromium: all five pages mount **byte-identical slider values** before and after | ✅ done |
| 2026-08-19 | **A move inside a file is still declarable, and that is what kept both parity gates.** `gsparity.js` learned two `cut`s with an empty `gone` list — a name that moved is still in `script.gs`'s top-level set, so check 3 says so on its own — and `modparity.js` a declared `AmrSlide` edit. Cost, stated where the edit is: `slidefit.js` drives the legacy `SlideExport.html`, so it no longer exercises that module's first five lines; `threefiles`, `pageparity` and `bgrender` all boot `app.html` whole, so the frame is still rendered from those values. `tests/deckstatic.js` loads `Config.gs` beside `Deck_Recipe.gs` now — one global scope is what the runtime does anyway | ✅ done |
| | **The current and prior year are still written out by hand in about sixty places** — `AmrSegSlide`'s `CY_YEAR`/`PY_YEAR`, `AmrRmxSlide`'s and `AmrFuelExec`'s table headers, both fuel pages' titles and hint text, and the QlikView guides' sample tables, where they are *illustrations* and should not move with the calendar at all. Hoisting half of them would leave one knob that changes some of the headings and not others, which is worse than no knob. Written up as chunk 23 | ☐ |
| 2026-08-19 | **Chunk 20 — `PV.toNum_` stops dropping accounting negatives.** `'(1,234)'` read as **`0`**, not `-1234`: the strip left the brackets, `parseFloat` gave `NaN`, and `NaN` became `0`, so the figure was **dropped from every sum it landed in rather than mis-signed** — which is exactly why nothing ever looked wrong. Both copies in the family (`PV` and `PVLOOK`, byte-identical, and `PVLOOK`'s ranks the mapping check **by money impact**, so a dropped negative there reorders the list of what to fix first) now carry the rule the other four `toNum_` always had. **Not the unification chunk 15 forbade:** the percent rule that makes PV right and the others wrong is untouched — the two rules are about different inputs — and the fix cannot reach anything that is not a parenthesised *number*, which `tests/helpers.js` now pins with `(n/a)` and `(5%)` rows | ✅ done |
| 2026-08-19 | **The characterisation table corrected its author a third time.** `SASKRATES.toNum_('(5%)')` was predicted `0` — it is the one copy that does not strip `%`, so `'%12'` stops `parseFloat` dead — and the harness said **`-5`**: the parenthesis is rewritten to a minus *first*, giving `-5%`, and `parseFloat` is perfectly happy to stop at a **trailing** `%`. Two cells of this table were wrong when it was first written and a third was wrong when it was extended; reading six small helpers carefully is still not good enough | ✅ done |
| 2026-08-19 | **Chunk 21 — the mapping check is keyed on the schema it depends on, and there is still only one schema token.** `PV.gk_` mixed `SCHEMA_` into its cache key and `PVLOOK.gk_` did not, so bumping the schema stranded every Price & Volume table and left the mapping check serving a result **computed from rows of the old shape** — a stale answer that looks exactly like a correct one. `PV_Lookup.gs` cannot see `SCHEMA_` (it is another IIFE), so PV **exports** it, which is the same move and the same argument as `PV.readTab` two lines above: a second `'v5'` literal is the thing nobody remembers to bump on the one occasion it matters. Stated in advance rather than discovered: every existing `PVLOOK` entry is now unreachable — one miss, then a rebuild, for one page's check | ✅ done |
| 2026-08-19 | **The two keys are still not identical, and that is the safe direction.** PV keys on the raw source stamp; `PVLOOK` keys on `APP_getGen_`, which is that stamp **plus `APP_CODE_BUILD`** — so a code push already invalidated the mapping check while leaving PV's tables alone. Extra rebuilds, never a stale read, so it is left as it is and written down instead. `tests/pvlookup.js` gained check 5, which **runs** the key builder rather than reading it: the schema read crosses an IIFE boundary, which is the half that can actually throw, and on the pre-fix tree it fails printing the real key | ✅ done |
| 2026-08-19 | **Chunk 23 — the year is data, not a setting, and it was baked into four data contracts.** The chunk was written up as "~60 display literals". It was that plus the part that matters: **AGG Fuel Recovery summed `market|2026|month` buckets**, so the first export of a new year would have found nothing and published a **complete table of zeroes** under headings naming the year that had gone. **Ready-Mix** asked for the columns `'2026 vol'` and `'2025 net sales ex va (cad)'` — `col_` returns −1, `toNum_` turns the missing cell into 0, same zeroes. **TP01** read the uploaded QlikView file with `headers.indexOf('2026 Volume')`, and the workbook still downloads. **The QlikView sync's** alias map covered exactly two years. Not one of them would have thrown, and **every fixture in `tests/` is a 2026 workbook**, so nothing would have caught it | ✅ done |
| 2026-08-19 | **The fix was already in the repo, twice, and both times it was written down.** `RFSC_Backend.gs` has carried `cy`/`py` off `yMax` since it was written and says so in its header; `PV_Backend.gs`'s reader has matched `"#### Volume"` by pattern with the comment *"the header carries the year, so nothing here has to be edited when the file rolls over"* — four lines from an upload path that listed `'2025 Volume'` and `'2026 Volume'` as required columns. Chunk 23 is the rest of the suite copying its own good example: `yearPair_` in Ready-Mix, `cy`/`py` on FSC's bundle, `iYearCol` in TP01, patterns instead of six literal aliases in the sync. **The RMX Fuel Recovery page's list of "everything that differs from the AGG page" is one bullet shorter** — the two are true clones again | ✅ done |
| 2026-08-19 | **`tests/yearroll.js`** — the whole stack against a **2031** workbook, because a fixture from this year cannot tell "reads the data" from "assumes the calendar". It checks the FSC payload names 2031/2030 and that its figures are the sheet's rather than zero, that `yearPair_` picks the two newest of three years, that the sync's alias maps any year, that TP01's finder takes the newer of two and −1 when absent, and that **every four-digit year left in `app.html` outside a comment is a guide sample**. Mutation-tested three ways: restoring the literal sum, restoring TP01's `indexOf`, and letting one `2026` back into a heading — each fails naming the exact regression | ✅ done |
| 2026-08-19 | **A knob was the wrong answer, and §C says so.** Chunk 22 left a note in §C arguing that hoisting half the year literals would be worse than none. That was right, and the conclusion was not "add a better knob" but "there is nothing to configure": the year is in the workbook's own column names, so a tunable could only ever be a way to **disagree with the data**. §C carries that reasoning where the next person to look for a year setting will find it | ✅ done |
| 2026-08-19 | **`tests/pageswitch.js` prints the stack of an uncaught page error, not just its message.** "Cannot read properties of null (reading 'style')" on the landing page has appeared twice, both times inside a full-suite run where the machine is loaded, and has not reproduced in ~20 runs since — including under deliberate concurrent load. It predates this session's changes (it appeared on the first baseline run, before anything was touched). A message with no stack is a report nobody can act on, so the harness now carries five frames; the next occurrence will name its own line | ☐ open, and now diagnosable |
