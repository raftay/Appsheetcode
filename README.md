...
| 2026-08-28 | **The SAP / USGAAP KPI strips read as the current month on the two pages that upload the workbook.** The EBITDA workbook is a *closed-month* statement — this calendar month minus one — and nothing inside it says so. The Overview already reads it that way (`kpiMonth`, one month back from the anchor, and the cards are on the Prev-month picks only), but **Price & Volume** and **RMX · Product Segment** did not: both drew the strip beside their own Report-month picker and labelled it only `month to date`, so a July card sat under an August heading with nothing on screen to say which month it was. Worse, Price & Volume ran the workbook's file-name check against **the picker** (`STATE.month || STATE.latestMonth`), so a correct July file was reported as the wrong file whenever the page sat on the running month, and Product Segment made **no check at all**. `AmrKpi.closedMonth()` / `closedLabel()` are now the one place that calendar rule lives; both strips name the closed month in their meta line, Price & Volume checks the file name against it instead of the picker, and Product Segment gained the same caution under each file input. **No KPI value moved** — the workbook was always the closed month's; what changed is that the pages now say so and check against it. | ✅ |
