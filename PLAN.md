...
**Status: EVERY CHUNK IN THIS FILE IS DONE — nineteen for the merge, plus 20-23 after it — on `merging-files`. The script project is `script.gs`, `app.html` and `appsscript.json`, and nothing else — and that is now *tested* rather than asserted: `tests/threefiles.js` runs the whole application out of a directory holding only those three.** 37 files became 3. `app.html` is 1.14 MB and holds the runtime, thirteen shared modules and all ten pages; `script.gs` is 547 KB in eleven sections. All **28** harnesses run and are green (26 before chunk 22 added `tunables.js` and chunk 23 added `yearroll.js`).
...
**[§8](#8-the-chunks) is complete through 23.** Chunks 20 and 21 — chunk 15's two findings — and chunk 23, chunk 22's one, are all done; there is no open code left in this file. What remains is the three editor-run items in the block below, and nothing off-platform can do any of them. Everything else here is the record of how this got here.
