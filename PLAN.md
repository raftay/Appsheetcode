...
**Status: EVERY CHUNK IN THIS FILE IS DONE — nineteen for the merge, plus 20-25 after it — on `merging-files`. The script project is `script.gs`, `app.html` and `appsscript.json`, and nothing else — and that is now *tested* rather than asserted: `tests/threefiles.js` runs the whole application out of a directory holding only those three.** 37 files became 3. `app.html` is 1.14 MB and holds the runtime, thirteen shared modules and all ten pages; `script.gs` is 547 KB in eleven sections. All **26** harnesses run and are green. The count went 26 → 28 when chunks 22 and 23 added `tunables.js` and `yearroll.js`, then back to 26 when `regress.js` and `pvcheck.js` were deleted — see the note below.
...
> are both the file `app`: the project cannot hold both, the editor refuses the second, a
> `clasp push` is rejected, and `HtmlService.createTemplateFromFile('app')` — the call `doGet`
> depends on — becomes ambiguous. The plan below was written when this file was called
> `app.gs`; every reference has been updated, and the title's "one `app.html`, one `app.gs`"
> is the same two files under the name the platform allows. The HTML keeps `app`; the server
> side is the one that moved.

**[§8](#8-the-chunks) is complete through 25.** Chunks 20 and 21 — chunk 15's two findings — chunk 23, chunk 22's one, and chunks 24 and 25 are all done; there is no open code left in this file.

> **Two harnesses were deleted, and the reason is a rule worth carrying.** `regress.js` and
> `pvcheck.js` diffed the pre-extraction pages against `AmrFuelExec` / `AmrPvSlide`, with the
> old page staged **by hand** into `$OLD_DIR`. The commits they name (`6400026`, `cc3adc9`)
> are not reachable from this repo any more, and the newest copies that ARE reachable are
> one-line delegations back to those same modules — so restaging would have produced a
> comparison that passes whatever either side does. Their claim had also been superseded on
> purpose by chunks 16-19 and 22-23. **A gate whose second side has to be assembled by hand
> is a gate that stops running**; `gsparity.js`, `modparity.js` and `apphtml.js` stage from a
> commit and all three still work. `modparity.js` is now the only proof for those two
> modules, and its header says so.

> **Chunk 25 is the one to read if you are about to dismiss something as a flake.** The
> previous session left "Cannot read properties of null (reading 'style')" open: seen twice
> under load, not reproducible in ~20 runs, and — reasonably — judged to predate that
> session's changes. What it actually was is a `google.script.run` answer landing after its
> page had been unmounted, reported against whatever page was mounted by then. The fix that
> mattered was made BEFORE the diagnosis: teaching `pageswitch.js` to print five stack
> frames. Without them the next occurrence would have been one more unactionable line. What remains is the three editor-run items in the block below, and nothing off-platform can do any of them. Everything else here is the record of how this got here.

**Three items are still owed and none of them is code — all three need somebody in the Apps Script editor:** `APP_verifyPermissions()` has never been run, no real deck has ever been built against the live deployment, and `DECK_status` is being kept until that build says whether the Publish stage needs it. **A fourth is one look at a sheet:** chunk 20 fixed `PV.toNum_` dropping parenthesised negatives, and whether the Price & Volume source actually carries any decides only whether anyone notices — a no-op if it has none, correctly counted figures if it has some.

> **The parity harnesses are no longer a complete account, and that is by design.** `pageparity.js`, `cssparity.js` and `modparity.js` compare `app.html` against the app it replaced. Chunks 16–19 changed §B, both fuel pages, two §E modules and §D on purpose, and chunks 22–23 changed `AmrSlide`, `AmrFuelExec`, `AmrSegSlide` and `AmrRmxSlide` — each declared rather than waved through — so what those three still prove is narrower than it was: `modparity` carries the §E changes as **declared edits** and proves the rest verbatim, and `pageparity` still proves what a COLD visit renders, with one declared narrowing since chunk 24: the Deck Builder's layout cell is a dropdown now, so it is compared as the layout NAME it shows rather than byte for byte, and every other byte of those rows still is. `fuelcache.js`, `slidecss.js`, `logging.js`, `settings.js` and `pageswitch.js` cover the behaviour that is new. Read a green parity run as "the port is still faithful where it is still a port", not as "nothing has changed".
...
> 4. **There is no open code left in this file.** Chunks 20-25 are done. What remains is the
>    three editor-run items in the status block at the top, none of which anything
...
5. [The shape of `script.gs`](#5-the-shape-of-scriptgs)
...
> **one `app.html` and one `script.gs`,** plus `appsscript.json`. Nothing else.
...
**Re-confirmed at chunk 12 against all 181 names, and again against the merged `script.gs`: still
...
**`script.gs` cannot sit in the live script project next to the files it replaces.**
...
controls. Dropping a complete `script.gs` into the live project would silently re-point the live
...
2. Merge the `.gs` files **last**, as a single atomic cutover commit — delete all 16, add
   `script.gs` — at a point where `app.html` is already known good.
...
`script.gs` gains `APP_verifyPermissions()` — run it from the editor after pasting the file in,
...
> user. `APP_verifyPermissions()` turns that into one editor run, and `script.gs` §4's `CHECKS`
...
- ~~**`RMX_whoWins` stays, and very nearly did not.**~~ **Deleted after chunk 19**, and not
  ...
  construction and every stack names `script.gs`. The fourth, the backend build stamp, already
  ...
  `script.gs` deleted this function **by accident**, and every syntax and structural check passed.
...
| 20 | **`PV.toNum_` drops accounting negatives** | **Fixed.** `PV.toNum_('(1,234)')` was `0` where every other copy gives `-1234` — the strip left the parentheses, `parseFloat` gave `NaN`, `NaN` became `0`, and the figure was **dropped rather than mis-signed**, which is why nothing ever looked wrong. Both copies in the family (`PV` and `PVLOOK`, byte-identical) now carry the rule the other four always had. **This is not the unification chunk 15 forbade** — the percent rule that makes PV right and the others wrong is untouched, because the two rules are about different inputs, and the fix cannot reach a value that is not a parenthesised NUMBER (`(n/a)` still reads `0`). `tests/helpers.js`'s table moves with it: two cells, `0 → -1234` and `0 → -99.5`, plus two new rows pinning the limit. | `helpers.js` (mutation-tested), and the sheet check below | ✅ |
| | **The one thing chunk 20 could not do off-platform, and what it means.** The chunk said "check the sheet first" — nothing here can open the Price & Volume workbook, so whether the source actually carries parenthesised negatives is still unknown. What that changes is only **whether anyone will notice**: if the source has none, the fix is a no-op on every figure; if it has any, those figures were being silently dropped from sums and are now counted. There is no third outcome, because a parenthesised number is not a value any column of this workbook wants read as zero. Worth one look at the raw tab next time somebody is in the sheet, to know which of the two happened. | ☐ |
| 21 | **`PVLOOK.gk_` ignores `SCHEMA_`** | **Fixed, with ONE token rather than two.** `PV.gk_` mixed `SCHEMA_` into its key and `PVLOOK.gk_` did not, so a schema bump stranded every Price & Volume table and left the mapping check serving a result **computed from rows of the old shape** — a stale answer indistinguishable from a correct one. `PV_Lookup.gs` cannot see `SCHEMA_` (another IIFE), so PV **exports** it — the same move, and the same argument, as `PV.readTab` two lines above it: two copies of one rule is how it keeps coming back, and a second `'v5'` literal is the thing nobody bumps. As stated in advance: every existing `PVLOOK` entry is now unreachable — one miss, then a rebuild, for one page's check. **The two keys are still not identical, deliberately**: PV keys on the raw source stamp, `PVLOOK` on `APP_getGen_`, which is that stamp *plus* `APP_CODE_BUILD`. That asymmetry only ever causes an extra rebuild, never a stale read, so it is left alone and written down. | `helpers.js` (source shape, mutation-tested two ways) + `pvlookup.js` check 5, which RUNS the key builder — the read crosses an IIFE boundary, which is the half that can actually break | ✅ |
...
| 23 | **The year is data, not a setting** | **Done, and bigger than the chunk said.** The write-up counted ~60 display literals; the year was also baked into the DATA CONTRACT in four places, each of which fails **silently** on the first export of a new year. AGG Fuel Recovery summed `market|2026|month` buckets, so a 2027 export would have published a **complete table of zeroes** under headings naming a year that had gone. Ready-Mix resolved CY/PY by asking for the columns `'2026 vol'` / `'2025 net sales ex va (cad)'` — `col_` returns −1, `toNum_` turns the missing cell into 0, same zeroes. TP01 read the uploaded QlikView file with `headers.indexOf('2026 Volume')` — same −1, and the workbook still downloads. The QlikView sync's alias map covered exactly 2025 and 2026. **Not one of them would have thrown, and no fixture in `tests/` is from any year but 2026.** Every backend now reads the pair off the workbook's own column names (`yearPair_` in RMX, `yMax` in FSC, the `/^(\d{4})\s+volume$/` the PV reader always used) and sends `cyYear` / `pyYear` with every payload; every heading, title and status line prints what it was sent; and the QlikView guides' sample rows are left alone on purpose, because they illustrate the shape of an export beside made-up plant codes. **There is nothing to configure — a knob would only be a way to disagree with the data — which is why §C holds no year.** | `yearroll.js` (new: a **2031** workbook, end to end, mutation-tested three ways) + the whole suite, 26 green | ✅ |
| 24 | **The recipe's layout column is a dropdown, not a code push** | **Done.** `DECK_RECIPE`'s `layout` was the only answer to "which template slide is this row built from", and changing it meant editing the array and pushing — for a decision that is nobody's idea of code. The Plan stage now offers every **report** layout the template actually has, per row, and the choice is stored SHARED in one Script Property (`DECK_CONFIG.PROP_LAYOUTS`). **Only the differences are stored**: a row on its recipe layout has no key, because copying all 43 rows in the first time anyone touched one would freeze the array and the next person to re-point a slide in the code would change nothing and have no way to see why. The row carries what it is BUILT from and what the recipe NAMES, so "back to default" is offerable and a deliberate override is tellable from an accidental one. **Changing a layout drops that row's picture and requeues it** — `renderOne` takes the capture's height and pixel width from the layout's `{{IMAGE}}` box, so a picture taken for a full-width slot is the wrong shape for a half-width one and nothing downstream would notice. Validation sits at SAVING, not reading: `DECK_getRecipe` is Plan's only request and must stay instant, so it makes no Slides call; `DECK_setLayout` already has a reason to open the template. An override for a row that has left `DECK_RECIPE` is reported rather than ignored; an unparseable store falls back to the recipe rather than locking the page out of Plan. | `deckstatic.js` +11 checks (default, override, override-equals-default, orphaned key, unparseable store) — and it now evaluates `Deck_Backend.gs` beside the recipe, which is what one global scope actually does; four declared edits in `gsparity.js`; the layout cell narrowed in `pageparity.js`; whole suite, 26 green | ✅ |
| 25 | **The intermittent landing-page null `.style`, root-caused** | **Done — and it was never the landing page.** The open note at the end of the last session said this had appeared twice under load, had not reproduced in ~20 runs, and predated that session's changes; the harness was taught to print five stack frames so the next occurrence would name its own line. It did. It is RMX's `renderUnmapped`: `loadUnmapped` fires a `google.script.run`, the switcher unmounts the page, the answer lands, `$('mapHost')` is null and `host.style` throws out of a callback nothing is catching — reported against whatever page is mounted BY THEN, which is why it looked like a landing-page fault. **The Price & Volume copy of the same function has carried the two-line guard since the Inventory Report fix; this one was missed**, along with both failure handlers. Chunk 24's extra bytes shifted the timing enough to make it deterministic, which is the only reason it was catchable at all. **The lesson is the diagnostic, not the guard:** a message with no stack is a report nobody can act on, and adding the frames is what closed a bug that had already survived two sessions. | `pageswitch.js` — red before, green after, 21 switches with no uncaught error | ✅ |
...
- **It follows `APP_EXTRA_SOURCES`, not a list of pages.** `script.gs` already declares which pages
...
- **12 stay silent and each has its reason written down** — a cache handle, a cache read, a
  ...
  expected miss). The census lives in `script.gs` §2, beside the helper, not in a commit message.

- **A harness that slices one region out of `script.gs` was modelling the scope wrongly, and this
  ...
  **one** global scope — the entire premise of the file — so `APP_log` in §2 is reachable from
  ...
  like a broken build and is a missing global. `tests/scriptgs.js` installs it now, through `load()`
...
of `script.gs` by namespace and runs each against a table of inputs, recording *what the suite
...
- **`tests/apphtml.js` is the client-side twin of `scriptgs.js`,** and it draws the same
...
- **Three lists now have to name the same ten pages** — `APP_PAGES` in `script.gs`, `AMR_PAGES`
  in §D, and the `§P` templates — and nothing made them agree. `merge.js` check 10 does.
  Drift is silent and asymmetric: missing from `script.gs` and the switcher offers a link that
...
- **`gsparity.js` needed a new edit kind.** `replace` swaps one exact string; a rewritten
  ...
  **literal** — reading it back out of `script.gs` would make the check unable to fail. `doGet`
...
- **The merge really was ordered concatenation, and now there is a gate that says so.**
  ...
  checks each region appears in `script.gs` verbatim and in the [§5](#5-the-shape-of-scriptgs) order.
...
- **Two files contain a lone `\r` used as a line terminator.** `FSC_Backend.gs` and
  ...
  repo was "mixed CRLF/LF"; it was actually mixed three ways. `script.gs` normalises all three to
...
- **The seven harnesses that read a `.gs` read a REGION of `script.gs` now, not the whole file**,
  through the new `tests/scriptgs.js`. That distinction is the point. Several of them assert on
  *source text* — "PV takes its generation from the sheet", "no local header-is-row-1 reader is
  back in `PV_Lookup`" — and against the whole of `script.gs` those regexes pass if **any** of the
...
- **`tests/rmxcost.js` spliced a key onto the RMX IIFE's return using an anchor with a literal
  `\r\n` in it.** `RMX_Backend.gs` was CRLF; `script.gs` is LF, so that anchor stopped matching
...
- **`GmailApp` is referenced zero times in the entire suite** — only `MailApp`, in
  ...
  the reasoning for every one is in `script.gs` §4's banner, because JSON takes no comments.
...
- **Two sentences in `app.html` are now untrue, and chunk 12 made them untrue.** Both are
  ...
  - the Price & Volume cross-year notice tells the user to "paste the current `PV_Backend.gs`
    into the Apps Script" — a file that no longer exists. It is `script.gs` now.
...
- **What is left is the server.** Chunk 12 merges the 16 `.gs` files, and §2's asymmetry is
  why it comes last: `script.gs` cannot coexist with the files it replaces, so it is one atomic
  commit — delete all 16, add `script.gs` — taken at a point where `app.html` is already known
...
- **`app.html` is ~1.13 MB, not the ~1.0 MB estimated in chunk 0.** Measured with all ten
  ...
  did not include. ~~**`script.gs` ~515 KB.**~~ **Measured at chunk 12: 542,273 bytes, 11,388
...
All four now live in `script.gs` **§11**, split off from the `QLIKSYNC` engine in §5 on the file's
...
Run after chunk 19, over every top-level declaration in `script.gs`, with comments and string
...
- The `RMX_Backend.gs` "legacy names" wrappers — `getMarkets`, `getKeys`, `getExtras`,
  ...
  they are all in `script.gs` §7 and read as ordinary top-level functions.** They cannot simply be
...
- **Do not flip a file's line endings.** The repo is mixed **three ways**, not two: most
  ...
  **lone `\r`** as a line terminator. `app.html` and `script.gs` are written fresh and are **LF**
...
- **Run the harnesses in `tests/` before and after each chunk.** They are the only proof
  ...
  - `tests/rmxfixture.js` *(after chunk 12)* — not a harness; the one Ready-Mix model the two
    ...
    markup.
  - `tests/gsparity.js` *(chunk 12, retires at chunk 13)* — every region of `script.gs` is
  ...
  - `tests/threefiles.js` — the project really is three files. Copies `script.gs`, `app.html` and
    `appsscript.json` into an empty directory and runs the app out of THAT: evaluates `script.gs`
  ...
  - `tests/logging.js` *(chunk 18)* — the silent-catch census and the entry points. Every
    `catch (…) {}` left in `script.gs` is listed with its reason, so a new one fails with "decide
  ...
  - `tests/scriptgs.js` *(chunk 12)* — not a harness; the shared slicer the seven `.gs`-reading
    harnesses use to read **one region** of `script.gs`. Read its header before repointing
...
| **`script.gs` is 542,273 bytes / 11,388 lines; the 16 sources were 528,246** *(chunk 12)* | `wc -c` before and after |
